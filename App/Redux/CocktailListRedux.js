...
export const request = (state) =>
  state.merge({ cocktailListLoading: true, cocktailListError:false, cocktailList: null });

// successful cocktail list lookup.
export const success = (state, action) => {
  const { cocktailList } = action;
  return state.merge({ cocktailListLoading: false, cocktailListError: false, cocktailList:cocktailList, cocktailListFiltered:cocktailList })
};

// failed to get the cocktail list
export const failure = (state) =>
  state.merge({ cocktailListLoading: false, cocktailListError: true, cocktailList: null });
...
export const setSearchInputState = (state, action) => {
  ...
  if(!searchInputState){
    return state.merge({searchInputState,cocktailListFiltered: state.cocktailList })
  }
