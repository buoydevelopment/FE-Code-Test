...
export default class APIConnector {
	...
	_request( uri, args = {} ) {
		...
		if ( !uri || uri instanceof String ) return logger.error( `No valid uri given for method ${method}` ) && this;
	...
	static _requestUpload( uri, options, uploadFormData ) {
		...
		return new Promise( ( ( resolve, reject ) => {
			...
			if ( options.headers[ 'X-Session' ] ) xhr.setRequestHeader( 'X-Session', options.headers[ 'X-Session' ] );
